...
class Window(QMainWindow):
    ...
    def initUI(self):
        ...
        self.pixmap2 = QPixmap(theGPS.getMapImage(theGPS.lat, theGPS.lng, self.theSlider.sl.value()))
...
class GPS(QWidget):
    def __init__(self):
        ...
        while not GPS.ifValidLatitude(self.lat):
            self.lat = input("Enter latitude: ")

        self.lng = input("Enter longitude: ")
        while not GPS.ifValidLongitude(self.lng):
            self.lng = input("Enter longitude: ")
...
class slider(QWidget):
   ...
   def valuechange(self, theGPS):
      self.label.setText(str(self.sl.value()))
      GPS.getMapImage(theGPS.lat, theGPS.lng, self.sl.value())
